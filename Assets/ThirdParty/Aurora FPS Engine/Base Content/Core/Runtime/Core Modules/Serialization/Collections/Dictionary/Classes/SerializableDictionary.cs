...
namespace AuroraFPSRuntime.CoreModules.Serialization.Collections
{
    ...
    public abstract class SerializableDictionary<TKey, TValue> : SerializableDictionaryBase, ICollection<KeyValuePair<TKey, TValue>>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, IDictionary<TKey, TValue>, IReadOnlyCollection<KeyValuePair<TKey, TValue>>, IReadOnlyDictionary<TKey, TValue>, ICollection, IDictionary, IDeserializationCallback, ISerializable
    {
        ...
        {
            SetKeys(new TKey[dictionary.Count]);
            SetValues(new TValue[dictionary.Count]);

            int index = 0;
            foreach (KeyValuePair<TKey, TValue> item in dictionary)
            {
                SetKey(index, item.Key);
        ...
        public override void OnAfterDeserialize()
        {
            if (GetKeys() != null && GetValues() != null && GetKeyLength() == GetValueLength())
            {
                ...
                {
                    if (!dictionary.ContainsKey(GetKey(i)))
                    {
        ...
        IEnumerable<TKey> IReadOnlyDictionary<TKey, TValue>.Keys
        {
            get
            {
                return ((IReadOnlyDictionary<TKey, TValue>)dictionary).Keys;
            }
        }
        ...
        IEnumerable<TValue> IReadOnlyDictionary<TKey, TValue>.Values
        {
            get
            {
                return ((IReadOnlyDictionary<TKey, TValue>)dictionary).Values;
            }
        }
        ...
        /// </summary>
        protected void SetKey(int index, TKey key)
        {
            GetKeys()[index] = key;
        }
        ...
        /// </summary>
        protected void SetValue(int index, TValue value)
        {
