...
{
    [HideScriptField]
    [AddComponentMenu("Aurora FPS Engine/System Modules/Settings System/Receivers/Y Input Smooth Settings Receiver")]
    [DisallowMultipleComponent]
    public sealed class YSmoothInputReceiver : SettingsReceiver
    {
        [SerializeField]
        [NotNull]
        ...
        protected override void OnLoad(object value)
        {
            ...
            float ySmooth = (float)value;
            if (invertValue)
            {
                float inversed = Mathf.InverseLerp(maxValue, minValue, ySmooth);
                ySmooth = Mathf.Lerp(minValue, maxValue, inversed);
            }
            smooth.y = ySmooth;
