...
{
    [HideScriptField]
    [AddComponentMenu("Aurora FPS Engine/System Modules/Settings System/Processors/Post Processing/AO Settings Processor")]
    [DisallowMultipleComponent]
    public sealed class AOSettingsProcessor : SettingsProcessor
    {
        [SerializeField]
        [NotNull]
        ...
        protected override object OnSave()
        {
            ...
            if (profiles != null && profiles.Length > 0)
            {
                for (int i = 0; i < profiles.Length; i++)
                {
                    ...
                    if (profile.TryGetSettings(out AmbientOcclusion ambientOcclusion))
                    {
                        ambientOcclusion.active = active == toggleOn;
                    }
                }
            }
            return active;
