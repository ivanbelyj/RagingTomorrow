...
{
    [HideScriptField]
    [AddComponentMenu("Aurora FPS Engine/System Modules/Settings System/Receivers/X Zoom Sens Input Settings Receiver")]
    [DisallowMultipleComponent]
    public sealed class XZoomSensInputReceiver : SettingsReceiver
    {
        [SerializeField]
        [NotNull]
        ...
        {
            base.Awake();
            Debug.Assert(playerCamera != null, $"<b><color=#FF0000>Attach reference of the player camera to {gameObject.name}<i>(gameobject)</i> -> {GetType().Name}<i>(component)</i> -> Pawn Camera<i>(field)</i>.</color></b>");
        }
        ...
        {
            Vector2 sensitivity = playerCamera.GetZoomSensitivity();
            sensitivity.x = (float)value;
            playerCamera.SetZoomSensitivity(sensitivity);
        }
